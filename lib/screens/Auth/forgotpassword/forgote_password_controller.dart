...
import 'package:red_restro/screens/Auth/otpverifyforgotpassword/otp_verify_screen_forgot.dart';
import 'package:shared_preferences/shared_preferences.dart';

class forgotePasswordController extends GetxController {
  final emailController = TextEditingController().obs;

  var forgotPasswordStatus = RxBool(false);
  Future<void> saveEmailforgote(String emailForForfote) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('email', emailForForfote);
  }

  Future<void> forgotePassword() async {
    final url = Uri.parse('http://10.0.2.2:3000/api/forgotpassword');
    ...
    try {
      ...
      if (statusCode == 200) {
        ...
        Get.to(() => const OtpVerifyScreenForgot(), transition: Transition.rightToLeft, duration: const Duration(milliseconds: 600));
